...
with st.expander("📘 주제 1: 수학적 사고 vs 인공지능적 사고", expanded=True):
    ...
    예를 들어 `2, 4, 6, 8...`이라는 수열을 보면 “2씩 증가하는 규칙이네”라고 판단하고 $a_n = 2n$이라는 식을 세웁니다. 이는 인간의 직관과 논리를 활용한 방식이죠.
    ...
    - x (항 번호): 3 → y (수열 값): 6  
        """)
    st.markdown("""
    AI는 “x가 1씩 증가할 때 y는 2씩 증가하네 → $y = 2x$”라는 규칙을 스스로 찾아냅니다.
    ...
    - **AI**는 규칙을 **데이터로부터 학습**합니다.
        """)
    # 📊 주제 2
with st.expander("📊 주제 2: AI는 어떻게 학습할까? — 회귀와 함수의 원리"):
    st.markdown("""
    ...
    """)
    st.latex(r"y = ax + b")
    st.markdown("예를 들어, 아래와 같은 데이터를 AI가 관찰했다면:")
        # 첫 번째 테이블 대신 목록으로 전개
    st.markdown("""
    - x: 1 → y: 3  
    - x: 2 → y: 5  
    ...
    이 데이터를 통해 AI는 수식을 $y = 2x + 1$로 학습할 수 있습니다.
        """)
    st.markdown("""
    이처럼 선형 회귀는 **직선으로 표현 가능한 관계**를 찾아내는 방법입니다.

    하지만 어떤 데이터는 **직선이 아니라 곡선**으로 표현됩니다. 예:
        """)
        # 두 번째 테이블 대신 목록으로 전개
    st.markdown("""
    - x: 1 → y: 1  
    ...
    → 이 관계는 $y = x^2$이라는 **2차 함수**로 설명할 수 있고, 이는 **다항 회귀**(polynomial regression)를 통해 학습됩니다.
        """)
    st.latex(r"y = ax^2 + bx + c")

    st.markdown("""
    - 여기서 **차수**(degree)는 함수의 최고 차항을 의미하며,  
    - 차수가 높아질수록 더 복잡한 패턴도 설명할 수 있습니다.
...
with st.expander("📐 주제 3: AI는 수식을 어떻게 만들까? — 반복 학습과 오차 최소화"):
    st.markdown("""
    AI는 수식을 스스로 만들기 위해 수많은 수식 조합을 시도해봅니다.  
    예를 들어 아래와 같은 형태의 수식을 가정합니다:
        """)
...
with st.expander("🧠 주제 4: 딥러닝 구조와 반복 학습의 특징"):
    st.markdown("""
    AI가 더욱 복잡한 문제를 해결하기 위해 발전한 기술이 **딥러닝(Deep Learning)**입니다.  
    ...
    ##### ❗ 과적합(Overfitting)
    층과 뉴런이 너무 많으면 훈련 데이터에만 잘 맞고 새로운 데이터에는 잘 맞지 않는 **과적합** 현상이 발생할 수 있습니다.
    ##### 🔁 반복 학습과 에포크(epoch)
